  ...
  try {
    ...
    if (googleScriptUrl) {
      try {
        ...
        const urlWithParams = `${googleScriptUrl}?email=${encodeURIComponent(
          email
        )}&phone=${encodeURIComponent(cleanPhone)}&source=${encodeURIComponent(
          source
        )}`;
        ...
        if (response.ok) {
          ...
          return NextResponse.json({
            success: true,
            message:
              "Dados cadastrados com sucesso! Você será avisado em primeira mão.",
          });
        } else {
          throw new Error(
    ...
    console.log(
      `📧 FALLBACK - Dados salvos localmente: ${email} | ${cleanPhone}`
    );
